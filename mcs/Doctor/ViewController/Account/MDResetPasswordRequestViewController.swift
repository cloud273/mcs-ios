...
class MDResetPasswordRequestViewController: McsFormViewController {
    ...
    @IBAction private func sendTap(_ sender: Any) {
        ...
        if usernameController.isValid {
            ...
            McsAccountResetPasswordRequestApi.init(.doctor, username: username, language: McsDatabase.instance.language).run() { (success, type, code) in
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "resetSegue" {
            let vc = segue.destination as! MDResetPasswordViewController
