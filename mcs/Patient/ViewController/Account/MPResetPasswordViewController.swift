...
class MPResetPasswordViewController: McsFormViewController {
    ...
    @IBAction private func sendTap(_ sender: Any) {
        ...
        if codeController.isValid && passwordController.isValid && rePasswordController.isValid {
            ...
            McsAccountResetPasswordApi.init(.patient, username: username, password: password, code: code).run() { (success, message, code) in
