...
class MPUpdatePasswordViewController: McsFormViewController {
    ...
    override func loadView() {
        super.loadView()
        passwordController = McsTextInputControllerPassword.init(passwordView, lPlaceHolder: "Current_password", pastable: false, require: true, delegate: nil)
    ...
    @IBAction private func sendTap(_ sender: Any) {
        ...
        if passwordController.isValid && newPasswordController.isValid && reNewPasswordController.isValid {
            ...
            if let token = McsDatabase.instance.token {
                ...
                McsAccountUpdatePasswordApi.init(.patient, token: token, password: password, newPassword: newPassword).run { (success, token, code) in
    ...
    class func create() -> MPUpdatePasswordViewController {
        return UIStoryboard(name: "Account", bundle: nil).instantiateViewController(withIdentifier: "updatePasswordVc") as! MPUpdatePasswordViewController
    }
